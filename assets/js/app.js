(function(){
  ...
  var bio=new IntersectionObserver(function(es){es.forEach(function(en){if(en.isIntersecting){en.target.style.width=en.target.getAttribute('data-bar')+'%';bio.unobserve(en.target);}});},{threshold:.5});
  ...
  var ctaSend=document.getElementById('ctaSend');if(ctaSend)ctaSend.addEventListener('click',function(){var n=document.getElementById('cName').value.trim();this.innerHTML=(n?'Thank you, '+n.split(' ')[0]+" — we'll be in touch ✓":'Request received ✓');});
})();
