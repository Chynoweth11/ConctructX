...
(function () {
    ...
    var progressFill = document.querySelector('#pageProgress span');
    function updateProgress() { if (!progressFill)
        return; var max = document.documentElement.scrollHeight - window.innerHeight; var pct = max > 0 ? (window.scrollY / max) * 100 : 0; progressFill.style.width = Math.max(0, Math.min(100, pct)) + '%'; }
    ...
    var contactForm = document.getElementById('contactForm'), ctaSend = document.getElementById('ctaSend'), formStatus = document.getElementById('formStatus');
    function setFormStatus(msg, type) { if (!formStatus)
        return; formStatus.textContent = msg; formStatus.className = 'form-status ' + (type || ''); }
    function setInvalid(el, on) { if (el)
        el.classList.toggle('is-invalid', !!on); }
    if (contactForm)
        contactForm.addEventListener('submit', async function (e) {
            e.preventDefault();
            var name = document.getElementById('cName'), email = document.getElementById('cEmail'), type = document.getElementById('cType'), notes = document.getElementById('cNotes');
            ...
            if (!type.value) {
                ...
                setInvalid(type, true);
            }
            if (errors.length) {
                setFormStatus('Please check your ' + errors.join(', ') + ' before sending.', 'err');
                return;
            }
            var original = ctaSend.innerHTML;
