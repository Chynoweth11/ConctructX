// @ts-nocheck
/**
 * ConstructX — hero residence geometry.
 *
 * Procedural stone/timber/glass model with canvas-generated textures, snow,
 * and dusk peaks. Split out from `hero-3d.ts` so it is fetched only when the
 * device has actually qualified for a real-time scene — visitors on reduced
 * motion, small screens, or weak hardware never download it.
 *
 * Exposes itself on `window` because the page loads plain scripts rather than
 * ES modules; `hero-3d.ts` waits for this global before building.
 */
(() => {
  function buildResidence(T, renderer, scene, residence) {
  var seed = 18;
  function rnd() {
  ...
  for (var gd = 0; gd < 3; gd++) box(0.34, 0.12, 0.065, glassMat, -5.05 + gd * 0.36, 0.76, 1.96);
  ...
  for (var sr = 0; sr < 18; sr++) shrub(-4.8 + rnd() * 9.4, 3.55 + rnd() * 1.05, 0.65 + rnd() * 0.7);
  for (var rr = 0; rr < 12; rr++) rock(-5.3 + rnd() * 10.5, 4.0 + rnd() * 1.1, 0.6 + rnd() * 1.3);
  ...
  var snowPoints = new T.Points(
    snowGeo,
    new T.PointsMaterial({ color: 0xffffff, size: 0.022, transparent: true, opacity: 0.48, depthWrite: false })
  );
  scene.add(snowPoints);

  }

  (window as any).ConstructXHeroScene = buildResidence;
})();
